...
import glob
import torch
from .basemodel import BaseDLModel
#epviz_dir = "/"
...
import txlstm_szpool, CNN_BLSTM
...
class TxlStmSzPool(BaseDLModel):

    def __init__(self):
        self.model = txlstm_szpool.txlstm_szpool()
        self.params = None

    def predict(self, input_data, fs=None, time=None):
        """
        Model expects
        * input_data: TimexChannels (ideally 19)
        * fs: sampling frequency (200Hz)
        * time: time vector (600s)

        Altogether: 120000 samples
        """
